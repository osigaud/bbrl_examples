    ...
    for _ in range(cfg.algorithm.max_epochs):
        ...
        while nb_agent_finished < pop_size:
            ...
            for idx_agent in range(n_to_launch):
                ...
                cem_rl.update_acquisition_actor(
                    acquisition_actors[idx_agent], idx_weight
                )
            ...
            acquisition_workspaces += [
                a.get_workspace() for a in acquisition_agents[:n_to_launch]
            ]
        ...
        logger.add_scalar(
            f"monitor/reward: {agents_creward.mean().item()}", n_interactions
        )
        logger.add_scalar(
            f"monitor/reward_best: {agents_creward.max().item()}", n_interactions
        )
...
def debug_train(cfg):
    ...
    pop_size = cfg.algorithm.es_algorithm.pop_size
    env_agent = env_agent = AutoResetGymAgent(
        make_gym_env,
...
@hydra.main(
    config_path=os.path.join(os.getcwd(), "run_launcher/configs/"),
    config_name="cem_rl_only_td3.yaml",
)

...
class CemRl:
    def __init__(self, cfg) -> None:
        ...
        self.centroid = copy.deepcopy(parameters_to_vector(actor_weights).detach()).to(
            "cpu"
        )
        ...
        self.param_transfer_agent = copy.deepcopy(
            self.rl_learner.get_acquisition_actor()
        )
    ...
    def train(self, acq_workspaces, n_total_actor_steps, logger) -> None:
        ...
        for i, workspace in enumerate(acq_workspaces):
            n_actor_all_steps += (workspace.time_size() - 1) * workspace.batch_size()
        ...
        if self.es_active:
            ...
            self.pop_weights = self.es_learner.ask(
                self.pop_size
        ...
        if self.rl_active:
            ...
            if (
                self.rl_learner.replay_buffer.size() < self.initial_buffer_size
            ):  # shouldn't access directly to replay buffer
                return
            selected_actors = random.sample(
                range(0, self.pop_size), self.n_rl_agent
            )  # take a half of the population at random
            ...
            for i in range(self.n_rl_agent):
                ...
                for _ in range(n_actor_all_steps // len(selected_actors)):
                    ...
                    train_workspace = self.rl_learner.replay_buffer.get(
                        self.rl_learner.cfg.algorithm.batch_size
                    )
                ...
                vector_param = torch.nn.utils.parameters_to_vector(
                    self.rl_learner.get_parameters()
                )

...
def synchronized_train(cfg):
    ...
    acquisition_agent, acquisition_workspace = NRemoteAgent.create(
        acquisition_agent, num_processes=n_processes, t=0, n_steps=cfg.algorithm.n_timesteps, **acquisition_args
    )
    ...
    for epoch in range(cfg.algorithm.max_epochs):
        ...
        # Get trajectories
        if epoch == 0:
            acquisition_agent(acquisition_workspace, t=0, n_steps=cfg.algorithm.n_timesteps, **acquisition_args)
        else:
            acquisition_workspace.copy_n_last_steps(1)
            acquisition_agent(acquisition_workspace, t=1, n_steps=cfg.algorithm.n_timesteps - 1, **acquisition_args)

        # Logging cumulated rewards:
        n_actor_steps = (acquisition_workspace.time_size() - 1) * acquisition_workspace.batch_size()

...
from pbt.pbt_utils import PBT_Agent
...
def synchronous_pbt(cfg):
    ...
    acq_env_agent = AutoResetGymAgent(make_gym_env, env_args, n_envs=n_env_per_process)
    ...
    acq_agent = TemporalAgent(Agents(acq_env_agent, (rl_population[0].get_acq_actor())))

    remote_agent, workspace = NRemoteAgent.create(
        acq_agent, num_processes=cfg.algorithm.n_process, t=0, n_steps=n_acq_steps, **rl_population[0].get_acq_args()
    )
    ...
    for epoch in range(cfg.algorithm.max_epochs):
        ...
        for i, pbt_a in enumerate(rl_population):
            ...
            while n_train_it_timesteps < cfg.algorithm.train_budget:
                ...
                if epoch == 0:
                    remote_agent(workspace, t=0, n_steps=n_acq_steps, **kwargs)
                else:
                    workspace.copy_n_last_steps(1)
                    remote_agent(workspace, t=1, n_steps=n_acq_steps - 1, **kwargs)
                ...
                if len(new_fitnesses) > 0:
                    logger.add_scalars("monitor/reward_pop", {f"{i}": new_fitnesses.mean().item()}, agent_time_step[i])
            ...
            logger.add_scalar("monitor/reward_best", agent_fitness[max_id].item(), agent_time_step[max_id])
